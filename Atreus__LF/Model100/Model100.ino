...
KALEIDOSCOPE_INIT_PLUGINS(
  ...
  //functions to tie to the leader key, brought from my atreus config
...
static void apostrophe(uint8_t seq_index) { Macros.play(MACRO( T(Quote)));}
static void underscore(uint8_t seq_index) { Macros.play(MACRO( D(LeftShift),T(Minus),U(LeftShift)));}
...
static const kaleidoscope::plugin::Leader::dictionary_t leader_dictionary[] PROGMEM =
 LEADER_DICT(
    ...
    { LEADER_SEQ(LEAD(0), Key_P),apostrophe}, //apostrophe
    { LEADER_SEQ(LEAD(0), Key_K),underscore}, //underscore
...
void setup() {
    ...
    QUKEYS(
    kaleidoscope::plugin::Qukey(0, KeyAddr(2, 12), Key_Hyper),   //qeu es ?, lo convierte en ctrl+cmnd+alt+shift para Karabiner
   
   kaleidoscope::plugin::Qukey(0, KeyAddr(2, 13), LSHIFT(Key_LeftGui) ),// letter l, when hold, becomes command+shift
